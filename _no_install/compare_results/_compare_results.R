...
test_that("tabulate_surf_periods(..., table_type = 'concise') replicates archived results.", {
    ...
    expect_identical(remove_attr(make_surf_period_control_tbl(
        ...
        remove_attr(
            filter_NAs(
                read_control_file(archived_tab_surf_periods_concise_prob_filepath))))
    ...
    expect_identical(remove_attr(make_surf_period_control_tbl(
        ...
        remove_attr(
            filter_NAs(
                read_control_file(archived_tab_surf_periods_concise_medians_filepath))))
})
...
test_that("tabulate_surf_stats(..., stat = 'avg_len') replicates archived results.", {
    ...
    ## Probabilistic
    expect_identical(remove_attr(
        make_surf_stat_control_tbl(read_control_file(archived_surfs_list_rda_filepath),
                                   stat = "avg_len")),
        remove_attr(filter_NAs(
            read_control_file(archived_tab_surf_stats_avg_len_prob_filepath))))
    ...
    expect_identical(remove_attr(make_surf_stat_control_tbl(
        ...
        remove_attr(filter_NAs(
            read_control_file(archived_tab_surf_stats_avg_len_medians_filepath))))
})
...
test_that("tabulate_surf_stats(..., stat = 'count') replicates archived results.", {
    ...
    ## Probabilistic
    expect_identical(remove_attr(filter_zeros(filter_NAs(
        make_surf_stat_control_tbl(
        read_control_file(archived_surfs_list_rda_filepath), stat = "count")))),
        remove_attr(filter_zeros(filter_NAs(
            read_control_file(archived_tab_surf_stats_count_prob_filepath)))))
